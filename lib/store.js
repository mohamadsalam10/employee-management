...
module.exports = {
  ...
  getBranchMeta, saveBranchMeta,
};

// Setup-defined overrides for a branch's display name and city.
// { "alquoz": { "city": "Dubai", "name": "Al Quoz" }, ... }
function getBranchMeta() {
  return read('branchmeta.json', {});
}
function saveBranchMeta(id, data) {
  const all = read('branchmeta.json', {});
  all[id] = { ...(all[id] || {}), ...data };
  write('branchmeta.json', all);
  return all;
}
