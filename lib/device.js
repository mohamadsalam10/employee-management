...
async function snapshot(target, channel) {
  return digestRequest(target, 'GET', `/ISAPI/Streaming/channels/${channel}/picture`, null, 'image/jpeg');
}
...
const camCache = new Map(); // target host -> { cams, ts }
const CAM_TTL = 5 * 60 * 1000;

function parseStreamingChannels(xml) {
  const blocks = xml.match(/<StreamingChannel>[\s\S]*?<\/StreamingChannel>/g) || [];
  const cams = [];
  for (const bl of blocks) {
    const id = (bl.match(/<id>(\d+)<\/id>/) || [])[1];
    if (!id) continue;
    const name = decode((bl.match(/<channelName>([\s\S]*?)<\/channelName>/) || [])[1]);
    const enabled = !/<enabled>\s*false\s*<\/enabled>/i.test(bl);
    cams.push({ channel: id, label: name || `Camera ${id}`, enabled });
  }
  // Keep one entry per camera: main streams end in "01" (101, 201, ...)
  let mains = cams.filter((c) => c.channel.endsWith('01'));
  if (!mains.length) mains = cams;
  return mains;
}

function parseChannelList(xml, tag) {
  ...
  for (const bl of blocks) {
    const id = (bl.match(/<id>(\d+)<\/id>/) || [])[1];
    if (!id) continue;
    const name = decode((bl.match(/<name>([\s\S]*?)<\/name>/) || [])[1]);
    cams.push({ channel: `${id}01`, label: name || `Camera ${id}`, enabled: true });
  }
  return cams;
}

// Ask the device/recorder for its own camera channels.
async function listChannels(target) {
  const hit = camCache.get(target.host);
  if (hit && Date.now() - hit.ts < CAM_TTL) return hit.cams;

  const attempts = [
    { path: '/ISAPI/Streaming/channels', parse: (x) => parseStreamingChannels(x) },
    { path: '/ISAPI/ContentMgmt/InputProxy/channels', parse: (x) => parseChannelList(x, 'InputProxyChannel') },
    { path: '/ISAPI/System/Video/inputs/channels', parse: (x) => parseChannelList(x, 'VideoInputChannel') },
  ];
  let lastErr = null;
  for (const a of attempts) {
    try {
      const res = await digestRequest(target, 'GET', a.path, null, 'application/xml');
      if (res.statusCode !== 200) { lastErr = `HTTP ${res.statusCode} on ${a.path}`; continue; }
      const cams = a.parse(res.buffer.toString('utf8'));
      if (cams.length) { camCache.set(target.host, { cams, ts: Date.now() }); return cams; }
    } catch (e) { lastErr = e.message; }
  }
  const empty = { cams: [], ts: Date.now(), error: lastErr };
  camCache.set(target.host, empty);
  return [];
}
...
module.exports = { getEventsForDay, snapshot, controlDoor, listChannels, todayInTz };
